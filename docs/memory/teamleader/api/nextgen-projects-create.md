---
url: https://developer.focus.teamleader.eu/docs/api/nextgen-projects-create
...
```
POST https://api.focus.teamleader.eu/projects-v2/projects.create
```
...
- titlestringrequired
  ...
  Also known as "budget". Value set by user to compare with price. Must be higher than or equal to zero, and lower than 100000000000 (100 billion). Only allowed for time\and\materials billing method. Currently only EUR is supported.
  ...
  Should not be after end\_date if provided
  
  Example: 2022-02-23
  
  end\_datestring
  
  Should not be before start\_date if provided
