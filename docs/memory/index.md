...
- **docs-* skill family**: `docs-bootstrap`, `docs-teamleader`, `docs-quickshell`, `docs-void` share one base+purpose — offline docs mirrors under `docs/memory/<product>/`, each with a `bin/sync-<product>-docs` entry point. Some are versioned (bootstrap, quickshell), some unversioned single mirrors (teamleader, void).
