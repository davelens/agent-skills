---
url: https://docs.voidlinux.org/config/openpgp.html
title: OpenPGP - The Void Linux Handbook
words: 211
---
GnuPG
...
As an alternative to GnuPG with smartcards, Void also ships openpgp-card-tools, a Rust based utility not reliant on GnuPG. It requires using pcscd for interacting with smart cards, so if you want to use it in parallel with GnuPG, you need to configure scdaemon to use the pcscd backend, as described above in "scdaemon with pcscd backend".
