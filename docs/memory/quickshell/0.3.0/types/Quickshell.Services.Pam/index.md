---
url: https://quickshell.org/docs/v0.3.0/types/Quickshell.Services.Pam/
...
PamContext currently only works with the auth type, as other types require root access to check.
...
Pam works with a set of modules that handle various authentication mechanisms. Some common ones include pamunix.so which handles passwords and pamfprintd.so which handles fingerprints.
