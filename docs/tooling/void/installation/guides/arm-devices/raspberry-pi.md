...
Sometimes rngd can cause CPU overload. To prevent this, inefficient entropy sources can be excluded and HWRNG device forced by creating /etc/sv/rngd/conf with the contents:
...
For Xorg, an Xorg configuration file may be needed, with the contents:
...
To enable I2C, add devicetreeparam=i2carm=on to /boot/config.txt, and bcm2708.vci2c_override=1 to /boot/cmdline.txt. Then create a modules-load(8) .conf file with the following content:
